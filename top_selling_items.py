...
def load_data():
    file_path = os.path.join('C:\\', 'Users', 'gokul', 'OneDrive', 'Documents', 'Infosys_SpringBoard_Project_Team1 Backup 08-01-2025', 'Infosys_SpringBoard_Project_Team1', 'hifi_eats', 'Infosys-Springboard-Team1', 'all_tables_data.xlsx')
...
def plot_top_selling_items(df, year, month):
    top_items = fetch_top_selling_items(df, year, month)
    if top_items.empty:
        st.error(f"No sales data available for {month}/{year}")
        return
